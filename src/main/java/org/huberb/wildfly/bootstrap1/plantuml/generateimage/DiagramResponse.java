...
class DiagramResponse {
    ...
    void sendMap(String uml) throws IOException {
        ...
        final Diagram diagram = blockUml.getDiagram();
        final ImageData map = diagram.exportDiagram(new NullOutputStream(), 0,
                new FileFormatOption(FileFormat.PNG, false));
        if (map.containsCMapData()) {
            final PrintWriter httpOut = response.getWriter();
    ...
    /**
     * Send a check of the plantuml text via http-response.
     *
     * @param plantumlText
     * @throws IOException
     */
    void sendCheck(String plantumlText) throws IOException {
        response.setContentType(getContentType());
        final SourceStringReader reader = new SourceStringReader(plantumlText);
        final DiagramDescription desc = reader.outputImage(
